...
int NXT_destroy(NXT *nxt) {
   ...
   return NXT_stopallmotors(nxt) || close(nxt->local_sock);
}
...
// return: 0 on success, non-zero on failure.
int NXT_disconnect(NXT *nxt) { return close(nxt->remote_sock); }
...
// return: 0 on success, non-zero on failure.
int NXT_play_tone(NXT *nxt, unsigned short freq, unsigned short time, bool ans,
...
int NXT_reset_motor_position(NXT *nxt, motor_port port, bool relative, bool ans,
                             unsigned char *status) {
   nxt_msg *msg;
   if (ans) {
