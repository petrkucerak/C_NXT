...
int NXT_driveforward(NXT *nxt, time_t s, power p, motor_port l, motor_port r) {
   motorstate lst = {
       l, truncate_power(p), REGULATION, SYNCHRONIZATION, 0, RUNNING, 0, 0, 0,
       0};
   motorstate rst = {
       r, truncate_power(p), REGULATION, SYNCHRONIZATION, 0, RUNNING, 0, 0, 0,
       0};
   ...
   return (NXT_stopmotor(nxt, l) || NXT_stopmotor(nxt, r));
}

// Drive backwards for s seconds at p power.
// If s is negative, drive  foward forever.
int NXT_drivebackward(NXT *nxt, time_t s, power p, motor_port l, motor_port r) {
   motorstate lst = {
       l, -truncate_power(p), REGULATION, SYNCHRONIZATION, 0, RUNNING, 0, 0, 0,
       0};
   motorstate rst = {
...
int NXT_turnleft(NXT *nxt, time_t s, power p, motor_port l, motor_port r) {
   motorstate lst = {
       l, -truncate_power(p), REGULATION, POWER_CONTROL, 0, RUNNING, 0, 0, 0,
       0};
   motorstate rst = {
       r, truncate_power(p), REGULATION, POWER_CONTROL, 0, RUNNING, 0, 0, 0, 0};
   if (NXT_set_motorstate(nxt, &lst, false, NULL) ||
       NXT_set_motorstate(nxt, &rst, false, NULL)) {
      return -1;
   }
...
int NXT_turnright(NXT *nxt, time_t s, power p, motor_port l, motor_port r) {
   motorstate lst = {
       l, truncate_power(p), REGULATION, POWER_CONTROL, 0, RUNNING, 0, 0, 0, 0};
   motorstate rst = {
       r, -truncate_power(p), REGULATION, POWER_CONTROL, 0, RUNNING, 0, 0, 0,
       0};
   if (NXT_set_motorstate(nxt, &lst, false, NULL) ||
       NXT_set_motorstate(nxt, &rst, false, NULL)) {
      return -1;
...
int NXT_setmotor(NXT *nxt, motor_port p, char v) {
   ...
   return NXT_set_motorstate(nxt, &st, false, NULL) || NXT_stay_alive(nxt);
}

// Stop a motor (set velocity to 0).
// return: 0 on success, non-zero on failure.
int NXT_stopmotor(NXT *nxt, motor_port p) { return NXT_setmotor(nxt, p, 0); }

// Stops all motors (sets velocity to 0).
// return: 0 on success, non-zero on failure.
int NXT_stopallmotors(NXT *nxt) {
   return NXT_stopmotor(nxt, MOTOR_A) || NXT_stopmotor(nxt, MOTOR_B) ||
