...
// return: 0 on success, non-zero on failure.
int NXT_play_tone(NXT *, unsigned short freq,
...
// Otherwise, status can be NULL.
// return: 0 on success, non-zero on failure.
int NXT_set_motorstate(NXT *, motorstate *, bool, unsigned char *);

// Gets the motorstate of a given port (MOTOR_A,MOTOR_B,MOTOR_C).
// return: 0 on success, non-zero on failure.
int NXT_get_motorstate(NXT *, motor_port, motorstate *);

// Sets the input mode of a given port to the type and mode specified.
// If ans, status will be set to the response of the NXT
// Otherwise, status can be NULL.
// return: 0 on success, non-zero on failure.
int NXT_set_input_mode(NXT *, sensor_port, sensor_type, sensor_mode, bool,
                       unsigned char *);
...
// Otherwise, status can be NULL.
// return: 0 on success, non-zero on failure.
int NXT_reset_motor_position(NXT *, motor_port, bool relative, bool ans,
                             unsigned char *);
